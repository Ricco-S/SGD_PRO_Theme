...
if ( ! function_exists( 'sgd_woocommerce_setup' ) ) :
  function sgd_woocommerce_setup() {
    add_theme_support( 'woocommerce' );
  }
endif;
add_action( 'after_setup_theme', 'sgd_woocommerce_setup' );
/*
...
// Add sgd wrappers
add_action('woocommerce_before_main_content', 'sgd_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'sgd_wrapper_end', 10);
function sgd_wrapper_start() {
  echo '<main id="main" class="site-main" role="main">';
}
function sgd_wrapper_end() {
  echo '</main>';
}
/*
Place a cart icon with number of items and total cost in the menu bar.
*/
function sgd_woomenucart($menu, $args) {
	...
	ob_start();
		global $woocommerce;
		$viewing_cart = __('View your shopping cart', 'sgd');
		$start_shopping = __('Start shopping', 'sgd');
		$cart_url = $woocommerce->cart->get_cart_url();
		$shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) );
		$cart_contents_count = $woocommerce->cart->cart_contents_count;
		$cart_contents = sprintf(_n('%d item', '%d items', $cart_contents_count, 'sgd'), $cart_contents_count);
...
add_filter('wp_nav_menu_items','sgd_woomenucart', 10, 2);
